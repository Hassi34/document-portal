...
def test_save_uploaded_files_filters_and_writes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_save_uploaded_files_uses_getbuffer(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
	monkeypatch.setattr(file_io, "SUPPORTED_EXTENSIONS", {".pdf"}, raising=False)
...
def test_save_uploaded_files_skips_unsupported(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_get_supported_extensions_default_when_missing(monkeypatch: pytest.MonkeyPatch) -> None:
