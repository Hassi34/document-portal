...
from src.schemas.ai.models import Metadata, SummaryResponse, ChangeFormat
...
class CompareResponse(BaseModel):
    ...
    rows: List[ChangeFormat] = Field(..., description="Row-wise structured comparison output")


class ChatIndexResponse(BaseModel):
    """Response schema for /chat/index."""
    session_id: str = Field(..., description="Chat session identifier used for indexing")
    k: int = Field(..., ge=1, description="Retriever top-k used when building the index")
    use_session_dirs: bool = Field(..., description="Indicates whether per-session directories were used")


class ChatQueryResponse(BaseModel):
    """Response schema for /chat/query."""
    answer: str = Field(..., description="Final answer generated by the RAG chain")
    session_id: Optional[str] = Field(default=None, description="Session identifier, if any")
