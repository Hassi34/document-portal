...
from src.utils.document_ops import load_documents, concat_for_analysis, concat_for_comparison
from src.utils.config_loader import load_config, get_supported_extensions
...
class FaissManager:
    ...
    def _exists(self) -> bool:
        ...
        return (
            (self.index_dir / f"{self.index_name}.faiss").exists()
    ...
    def load_or_create(self, texts: Optional[List[str]] = None, metadatas: Optional[List[dict]] = None) -> FAISS:
        ...
        if self._exists():
            ...
            try:
                ...
                if isinstance(index_dim, int) and isinstance(emb_dim, int) and index_dim != emb_dim:
                    log.warning(
                    ...
                    try:
                        (self.index_dir / f"{self.index_name}.faiss").unlink(missing_ok=True)  # type: ignore[arg-type]
                        (self.index_dir / f"{self.index_name}.pkl").unlink(missing_ok=True)    # type: ignore[arg-type]
                    except Exception:
...
class ChatIngestor:
    def __init__( self,
        ...
        try:
            ...
            self.temp_base = Path(resolved_temp); self.temp_base.mkdir(parents=True, exist_ok=True)
            self.faiss_base = Path(resolved_faiss); self.faiss_base.mkdir(parents=True, exist_ok=True)
    ...
    def built_retriver(
        ...
        *,
        ...
        try:
            ...
            cfg = load_config()
            if chunk_size == 1000:
                chunk_size = cfg.get("ai", {}).get("retriever", {}).get("chunk_size", 1000)
            if chunk_overlap == 200:
                chunk_overlap = cfg.get("ai", {}).get("retriever", {}).get("chunk_overlap", 200)
            if k == 5:
